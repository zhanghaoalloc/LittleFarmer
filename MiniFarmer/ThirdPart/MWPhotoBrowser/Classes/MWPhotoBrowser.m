...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Done", @"完成") style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
...
- (BOOL)canPhotoSelectedAtIndex:(NSUInteger)index willSelected:(BOOL)willSelected{
    BOOL value = YES;
    if (_displaySelectionButtons) {
        if ([self.delegate respondsToSelector:@selector(photoBrowser:canPhotoSelectedAtIndex:willSelected:)]) {
            value = [self.delegate photoBrowser:self canPhotoSelectedAtIndex:index willSelected:willSelected];
        }
    }
    return value;
}
...
- (void)selectedButtonTapped:(id)sender {
    ...
    if ([self canPhotoSelectedAtIndex:index willSelected:!selectedButton.selected]) {
